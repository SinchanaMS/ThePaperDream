  ...
  switch (sortby) {
    case "HIGH-TO-LOW":
      return sortedList.sort(
        (prod1, prod2) => prod2.price.discounted - prod1.price.discounted
      );
    case "LOW-TO-HIGH":
      return sortedList.sort(
        (prod1, prod2) => prod1.price.discounted - prod2.price.discounted
      );
    default:
      return sortedList;
  }
...
const searchForProduct = ({ searchFor }, products) => {
  const searchResults = products?.filter(
    (product) =>
      product.title.toLowerCase().includes(searchFor?.toLowerCase()) ||
...
export const getProductList = (filterState, products) =>
  applyFilters(
    filterState,
    sortByPrice,
    filterByRating,
